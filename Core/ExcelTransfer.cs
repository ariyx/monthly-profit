    ...
    static void WriteWorkbook(string file, List<object?[]> rows, List<object?[]> summary, string firstName, string secondName)
    {
        ...
        try
        {
            using (var archive = ZipFile.Open(temp, ZipArchiveMode.Create))
            {
                ...
                XElement Sheet(IEnumerable<object?[]> data)
                {
                    var sheetData = new XElement(S + "sheetData");
                    var rowIndex = 1;
                    foreach (var row in data)
                    {
                        var xmlRow = new XElement(S + "row", new XAttribute("r", rowIndex));
                        for (var columnIndex = 0; columnIndex < row.Length; columnIndex++)
                        {
                            var value = row[columnIndex];
                            var cell = new XElement(S + "c", new XAttribute("r", Column(columnIndex + 1) + rowIndex));
